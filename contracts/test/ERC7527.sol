...
contract ERC7527Agency is IERC7527Agency {
    ...
    function unwrap(
        ...
        require(
            _isApprovedOrOwner(_app, msg.sender, tokenId),
            "LnModule: not owner"
        );
        ...
        _transfer(address(0), payable(to), swap - burnFee); //_transfer 函数的第一个参数是 address(0)，表示从合约地址转移代币
        ...
        returns (uint256)
    {
        ...
        require(
            msg.value >= swap + mintFee,
            "ERC7527Agency: insufficient funds"
        );
    ...
    function getUnwrapOracle(bytes memory data)
        ...
        pure
        override
        returns (uint256 swap, uint256 fee)
    {
...
contract ERC7527App is ERC721Enumerable, IERC7527App {
    ...
    function burn(uint256 tokenId, bytes calldata)
        external
        override
        onlyAgency
    {
...
contract ERC7527Factory is IERC7527Factory {
    ...
    function deployWrap(
        ...
        bytes calldata
    ) external override returns (address appInstance, address agencyInstance) {
        appInstance = appSettings.implementation.clone(
